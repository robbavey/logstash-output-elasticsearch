...
Any previously loaded template should be overwritten when ILM is enabled,
If you have loaded a template before into Elasticsearch with the same version you must overwrite the template. Note: This feature only works with X-Pack Basic.

The default rollover_alias used is {logstash}

Note: If you modify the write_alias or the pattern, the index template will need to be rewritten as the settings index.lifecycle.name and index.lifecycle.rollover_alias change automatically in the template.

If no custom policy is defined, the default policy will be uploaded to Elasticsearch.

Compatibility
Note that this plugin requires an X-Pack Basic licence on the Elasticsearch server.
...
The write alias is the alias where indexes managed using Index Lifecycle Management will be written to
