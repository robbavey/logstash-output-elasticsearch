...
shared_examples_for 'an ILM enabled Logstash using index interpolation' do
  ...
  it 'should create the templates' do
    ...
    sleep(6)
    expect(@es.indices.get_template(name: "foo-one")["foo-one"]["mappings"][mapping_name]["properties"]["@version"]["type"]).to eq("keyword")
    expect(@es.indices.get_template(name: "foo-two")["foo-two"]["mappings"][mapping_name]["properties"]["@version"]["type"]).to eq("keyword")
    expect(@es.indices.get_template(name: "foo-three")["foo-three"]["mappings"][mapping_name]["properties"]["@version"]["type"]).to eq("keyword")
  end
